...
call exgsearch#register_hotkey( 5 , '<Space>'         , ":call exgsearch#toggle_zoom()<CR>"           , 'Zoom in/out project window.' )
call exgsearch#register_hotkey( 1 , '<CR>'            , ":call exgsearch#confirm_select('')<CR>"      , 'Go to the search result.' )
call exgsearch#register_hotkey( 2 , '<2-LeftMouse>'   , ":call exgsearch#confirm_select('')<CR>"      , 'Go to the search result.' )
call exgsearch#register_hotkey( 3 , '<S-CR>'          , ":call exgsearch#confirm_select('shift')<CR>" , 'Go to the search result in split window.' )
call exgsearch#register_hotkey( 4 , '<S-2-LeftMouse>' , ":call exgsearch#confirm_select('shift')<CR>" , 'Go to the search result in split window.' )
call exgsearch#register_hotkey( 7 , '<leader>r'       , ":exec 'Filter ' . @/<CR>"                    , 'Filter search result.' )
call exgsearch#register_hotkey( 7 , '<leader>fr'      , ":exec 'FilterFile ' . @/<CR>"                , 'Filter files in search result.' )
call exgsearch#register_hotkey( 7 , '<leader>d'       , ":exec 'ReverseFilter ' . @/<CR>"             , 'Reverse filter search result.' )
call exgsearch#register_hotkey( 7 , '<leader>fd'      , ":exec 'ReverseFilterFile ' . @/<CR>"         , 'Reverse filter files in search result.' )
"}}}
